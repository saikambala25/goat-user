...
app.post('/api/payment/stripe/create-session', authMiddleware, async (req, res) => {
  try {
    if (!stripe) {
      return res.status(500).json({ message: 'Stripe is not configured on the server.' });
    }

    const { items = [], total = 0, address } = req.body;
    if (!items.length) {
      return res.status(400).json({ message: 'No items provided for payment.' });
    }
    ...
    const origin = req.headers.origin || CLIENT_URL || `http://localhost:${PORT}`;

    // Save pending checkout on the user
    const user = await User.findById(req.user.id);
    if (!user) {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price_data: {
            ...
            unit_amount: amount,
            product_data: {
              name: 'Livestock purchase',
              description: `${items.length} item(s) from LivestockMart`,
            },
          },
          quantity: 1,
        },
      ...
      cancel_url: `${origin}/?payment=stripe-cancel`,
    });
...
// Stripe: finalize order after redirect
app.post('/api/payment/stripe/finalize', authMiddleware, async (req, res) => {
  try {
    if (!stripe) {
      return res.status(500).json({ message: 'Stripe is not configured on the server.' });
    }
    ...
    const user = await User.findById(req.user.id);
    if (!user || !user.pendingCheckout || !user.pendingCheckout.items || !user.pendingCheckout.items.length) {
      return res.status(400).json({ message: 'No pending checkout found for this user.' });
    }

    const checkoutData = user.pendingCheckout;
    ...
    user.pendingCheckout = undefined;
    await user.save();

    res.json({ success: true, order: newOrder });
  } catch (err) {
    console.error('Stripe finalize error:', err);
    res.status(500).json({ message: 'Failed to finalize Stripe order.' });
  }
});
